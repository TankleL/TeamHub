...
namespace TeamHub
{
    namespace Kernel
    {
        public abstract class DirItemObject: DiskNodeItem
        {
            ...
            public class PackOperator
            {
                public static sealed int BufferMaxSize    =   4096;
                ...
                ///     ----------------------------------------------
                /// </summary>
                public static NetBuffer PackClientInfo(Operation op,string src_path, string dest_path)
                {
                    try
                    {
                ...
                public static void UnpackClientInfo(NetDataPackage package, out Operation op, out string src_path, out string dest_path)
                {
                    int operation;
                    int pathLength;
                    try
                ...
                /// </summary>
                /// <returns></returns>
                public static NetBuffer PackServerInfo(DirItemServer dir_server)
                {
                    int operation;
                    int pathLength;
                    string path;
                ...
                public static void UnpackServerInfo(NetDataPackage package, out Operation op, out string path, out DateTime creation_time, out DateTime lastwrite_time, out DateTime lastaccess_time, out DiskNodeType node_type, out long node_size)
                {
                    int intTemp;
                    int longSize    =   sizeof(long);
                    byte[] bits = new byte[longSize];
                ...
                public static void UnpackServerInfo(NetDataPackage package,out Operation op,out DiskNodeItem item)
                {
                    string path;
                    ...
                    long node_size;
                    UnpackServerInfo(package, out op, out path, out creation_time, out lastwrite_time, out lastaccess_time, out node_type, out node_size);
                ...
                public static NetBuffer PackSubItems(Operation op, string path, DiskNodeItem[] subitems)
                {
	                ...
	                {	        
                        ...
                        {
                            PackServerInfo((DirItemServer)dni).Buffer(out tempBuffer);
                ...
                public static void UnpackSubItems(NetDataPackage package,out Operation op , out string path,out DiskNodeItem[] subitems)
                {
                    ...
                    try
                    {
                        ...
                        {
                            UnpackServerInfo(package, out opTemp, out tempNode);
                            subitems[i] = tempNode;
                        }
                    }
                    catch (Exception excp)
